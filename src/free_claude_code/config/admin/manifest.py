...
_NON_PROVIDER_FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        "FCC_ALLOW_REMOTE_ADMIN",
        "Allow Remote Admin",
        "runtime",
        "boolean",
        settings_attr="admin_allow_remote",
        restart_required=True,
        description="Allow Admin UI access from non-localhost. Disable to enforce local-only.",
    ),
    ConfigFieldSpec(
        "FCC_ADMIN_LOCAL_ONLY",
        "Enforce Local-Only Admin",
        "runtime",
        "boolean",
        settings_attr="admin_local_only",
        restart_required=True,
        description="When enabled, Admin UI is only accessible from localhost. Overrides Allow Remote.",
        advanced=True,
    ),
    ConfigFieldSpec(
        "PROXY_AUTH_ENABLED",
